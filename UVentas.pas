...
type
  TFVentasAgregar = class(TForm)
    ...
    procedure BBEncargosClick(Sender: TObject);
...
uses UDataModule, UFinalizarVenta, UPrincipal, UEncargos, UBuscarProducto;
...
begin
 if ((LEDescripcion.Text = '') or (LEPrecioU.Text = '') or (LECantidad.Text = '') or (LEAnillado.Text = '')) then
   ...
   begin
     if (TryStrToFloat(LEPrecioU.Text, aux2) = False) then
       begin
...
begin
   ...
   if ((LEIdProducto.Text = '') or (LEIdProducto.Text = '0')) then
     ...
     begin
       if (StrToInt64(LEIdProducto.Text) = 99) then
           begin
           ...
           begin
             ...
             Encontrado := False;
             Encontrado := DataModule1.FDTProducto.Locate('IdProducto', StrToInt64(LEIdProducto.Text));
...
procedure TFVentasAgregar.BBEncargosClick(Sender: TObject);
begin
   FEncargo.ShowModal;
end;
...
begin
    if ((LETotal.Text = '') or (LETotal.Text = '0') or (SGLineadeVenta.Cells[0,1] = '')) then
       ...
       begin
          ...
          DataModule1.FDQCuentasActivasEmpresas.Active := False;
          DataModule1.FDQCuentasActivasEmpresas.Active := True;
...
procedure TFVentasAgregar.SpeedButton1Click(Sender: TObject);
begin
